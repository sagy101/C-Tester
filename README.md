...
* **Declarative output contracts:** Complex checkers use a safe assignment-neutral JSON language over stdin, reference stdout, and actual stdout. Contracts can normalize text; extract numbers, labeled values, points, booleans, and anchored records; and assert equality, tolerances, sequences, or before/after exchange invariants. The interpreter does not execute generated code, formulas, or user regular expressions. Assignment-specific calculations should be captured from the compiled reference output, while reusable state invariants are expressed directly in JSON.
