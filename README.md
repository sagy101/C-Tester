...
* **Checker Manager:** Suggests generic semantic checker JSON, tests it against ground-truth outputs and generated counterexamples, saves per-question checker configuration, and audits samples selected by failure type as well as score. Verification is bidirectional: harmless semantic variants must pass, while wrong values, inverted booleans, missing facts, truncation, ordering errors, and distractors must fail. High-impact zero or parser-only deductions receive an independent challenger audit; disagreement stays uncertain. One-click calibration runs up to five audit/improvement rounds, regrades after promotion, preserves adjudicated behavior, reserves the final round as a holdout, and rolls back a regressing candidate. A standalone audit or Fake/Offline result cannot mark a checker verified.
