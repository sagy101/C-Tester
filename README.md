...
* **Checker Manager:** Suggests generic semantic checker JSON, tests it against ground-truth outputs and generated counterexamples, saves per-question checker configuration, and audits stratified samples of graded rows. One-click calibration runs up to three disjoint audit/improvement rounds, regrades after promotion, preserves all previously passed audit behavior at the per-input level, and rolls back a regressing candidate. Progress shows the active round, audit count, promotion/rejection, and final status. It can use Gemini or Fake/Offline. Post-scoring reviews that blame the checker feed directly into recalibration.
...
* **Declarative output contracts:** Complex checkers use a safe assignment-neutral JSON language over stdin, reference stdout, and actual stdout. Contracts can normalize text; extract numbers, labeled values, points, booleans, and anchored records; and assert equality, tolerances, sequences, or before/after exchange invariants. Boolean extraction is negation-aware: an alias directly followed by "not"/"n't" is inverted, so equivalent phrasings such as "isn't" and "is not" grade identically even when only one variant is configured, and draft tests replay a contraction-expanded reference output that must still pass. The interpreter does not execute generated code, formulas, or user regular expressions. Assignment-specific calculations should be captured from the compiled reference output, while reusable state invariants are expressed directly in JSON.
...
* **LLM Score Review:** After grading, opens a score/notes table. Use **Review All Attention Needed** for the recommended one-click path, or select visible rows manually. Reviews classify each deduction as caused by `student_code`, `checker_or_app`, or `unclear`. Checker-blamed rows surface a **Recalibrate From Reviews** action that opens Checker Manager with those findings. The LLM receives anonymized code, parsed failed inputs, raw student output, expected/reference output, grade text, Excel fields without `ID_number`, notes, active grading policy, and compile-repair metadata with IDs redacted. The Review Lab can use that context to generate a scratch fix, run it against all grading inputs, and retry with remaining failures.
